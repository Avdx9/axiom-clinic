  ...
  return (
    <section id="about" className="relative z-10 px-6 py-28 sm:px-10 lg:px-16">
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16">
        <RevealText className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-white/10 lg:aspect-auto">
          {/* Placeholder photography — grayscale + luminosity blend keep it
              inside the dark-mode palette regardless of the source image's
              original colors. Swap `src` for real clinic photography. */}
          <img
            src={ABOUT_IMAGE}
            alt="Interior of the Axiom clinic treatment room"
            loading="lazy"
            className="h-full w-full object-cover grayscale [mix-blend-mode:luminosity]"
          />
        ...
        <div className="flex flex-col justify-center gap-8">
          <GlassPanel className="p-8 sm:p-10">
            <RevealText className="mb-4 font-mono text-xs uppercase tracking-[0.3em] text-emerald">
              About Axiom
            </RevealText>
            ...
            <RevealText delay={0.2} className="mt-5 text-sm leading-relaxed text-muted sm:text-base">
          ...
          <RevealText delay={0.3} className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {CREDENTIALS.map(({ icon: Icon, label }) => (
              <div
                key={label}
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/40 px-4 py-3 backdrop-blur-md"
              >
                <Icon className="h-4 w-4 flex-shrink-0 text-emerald" aria-hidden="true" />
